...
public class SearchDialog extends JDialog {
	...
	void jbInit() throws Exception {
		...
		topPanel.add(jLabel1, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.NONE, new Insets(5, 14, 6, 11), 11, 0));
		topPanel.add(startFromField, new GridBagConstraints(1, 0, 1, 1, 1.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.HORIZONTAL, new Insets(5, 12, 0, 0), 257, 2));
		topPanel.add(jLabel2, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.NONE, new Insets(14, 14, 0, 12), 11, 4));
		topPanel.add(filesSearchField, new GridBagConstraints(1, 1, 1, 1, 1.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.HORIZONTAL, new Insets(14, 12, 0, 0), 257, 2));
		topPanel.add(findTextCheckBox, new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.NONE, new Insets(13, 14, 8, 0), 8, -5));
		topPanel.add(findTextField, new GridBagConstraints(1, 2, 1, 1, 1.0, 0.0, GridBagConstraints.WEST,
				GridBagConstraints.HORIZONTAL, new Insets(13, 12, 8, 0), 257, 2));
		topPanel.add(searchButton, new GridBagConstraints(2, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.NONE, new Insets(5, 25, 0, 33), 11, -2));
	...
	void searchButton_actionPerformed(ActionEvent e) {
		...
		if (startFromField.getText() == null || startFromField.getText().length() <= 0
				|| startFromField.getText().equals(".")) {
		...
		if (!startFileDir.isDirectory() || !startFileDir.exists()) {
			JOptionPane.showMessageDialog(this, "Invalid start directory specified.", "Error",
					JOptionPane.ERROR_MESSAGE);
			return;
		}

		if (filesSearchField.getText() == null || filesSearchField.getText().length() <= 0) {
			JOptionPane.showMessageDialog(this, "Invalid search mask specified. Use *,? to specify more files.",
	...
	/**
	 * Searches a directory for files. Throws an exception if it has to stop, just
	...
	/**
	 * These classes are used for pattern match. These are not mine, are stolen from
	 * a friend of mine Arthur Vitui. I use them, so that i won't be forced to use
	 * jdk's 1.4 regexp, to try to maintain 1.3 compatibility.
	 */
				...
				if (wildcardfit(wildcard, test, aux.pozw, aux.pozt) == 0) {
					do {
						...
						while ((wildcard[aux.pozw] != test[aux.pozt]) && (test[aux.pozt] != '\0'))
							(aux.pozt)++;
					} while (test[aux.pozt] != '\0' ? (wildcardfit(wildcard, test, aux.pozw, aux.pozt) == 0)
							: (0 != (fit = 0)));
				}
				if ((test[aux.pozt] == '\0') && (wildcard[aux.pozw] == '\0'))
					fit = 1;
